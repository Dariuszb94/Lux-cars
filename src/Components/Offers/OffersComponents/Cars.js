...
import FavouriteSet from '../../Offer/OfferComponents/FavouriteSet';
 ...
 */
  ...
  sort(filteredArray) {
    if (this.props.priceSort.priceSort === 'Ascending') { filteredArray.sort((a, b) => ((a.price > b.price) ? 1 : -1)); }
    if (this.props.priceSort.priceSort === 'Descending') { filteredArray.sort((a, b) => ((a.price < b.price) ? 1 : -1)); }
  }
        ...
        <CarList>
          {this.state.cars.map((car) => (
            <CarWithButtons key={car.id}>
              <StyledLink to={{ pathname: `/Offer/Vehicle-Features/${car.id}` }}>
